这是一个为你的Python DNS响应程序量身定制的、极其详细的开发计划。这个计划将遵循你提出的所有要求，并针对没有DNS基础的开发者进行详细解释，确保每一步都清晰、可执行。
...
| **2.4.4** | **数据包 PCAP 存储。** (把原始数据包保存下来，方便后续分析。) | 对于每一次触发：<br>1. 将**触发条件的数据包**保存为 `logs/<task_id>/<unique_id>_rule<rule_id>_query.pcap`。<br>2. 将**响应的数据包**保存为 `logs/<task_id>/<unique_id>_rule<rule_id>_response.pcap`。<br>`unique_id` 可以是自增序号或毫秒时间戳。 |
...
| **3.2.4** | **L7 (DNS) 字段展开。** | 在 **应用层 (L7)** 卡片内部，对 `Query`, `Answer`, `Authority`, `Additional` 等字段也使用二级折叠标签，以模仿 Wireshark 的交互逻辑。 |
...
| 步骤编号 | 任务描述 | 交互细节 |
| :--- | :--- | :--- |
| **3.3.1** | **任务列表。** | 实时读取 `logs/` 目录下所有的任务 ID (文件夹名)，以列表形式展示。点击任务 ID，展开该任务下的日志。 |
| **3.3.2** | **任务日志展示。** | 展开任务后，展示该任务下所有触发记录的列表 (例如：ID、规则编号、匹配时间、匹配域名)。 |
| **3.3.3** | **数据包查看器。** | 点击任一触发记录，弹出一个模态框或在侧边栏展示数据包详情。<br> - **展示内容：** Query 包和 Response 包的十六进制、ASCII 视图、以及分层解析视图 (类似 Wireshark)。<br> - **交互：** 点击分层解析视图的每一层，展示该层所有字段的详细信息 (例如，点击 **网络层**，显示 TTL、Protocol 等字段)。 |
| **3.3.4** | **任务删除功能。** | 在任务列表的每一行提供一个“删除”按钮，用于删除该任务 ID 对应的整个文件夹及其内部所有日志文件和 pcap 文件。 |
...
这个计划涵盖了从底层数据包处理到高层Web界面交互的所有细节，并特别针对规则的复杂配置和动态响应字段的赋值逻辑进行了详细说明，确保即使没有DNS基础，也能按步骤完成任务。